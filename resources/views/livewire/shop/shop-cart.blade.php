...
<div class="cart-box-main">
    ...
    @if (session()->has('success') || session()->has('fail'))
        ...
        <div class="fixed left-0 bottom-0 z-50 w-full md:w-1/3 bg-{{ $color }}-100 border-t-4 border-{{ $color }}-500 rounded-b text-{{ $color }}-900 pl-4 pr-1 pb-4 shadow-md"
            ...
            <div>
                <p class="w-full text-right cursor-pointer" x-on:click="show=false">Cerrar<i class="fas fa-times"></i></p>
                <div class="flex">
                    <div class="py-1">
                        <svg class="fill-current h-6 w-6 text-{{ $color }}-500 mr-4" xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 20 20">
                            <path
