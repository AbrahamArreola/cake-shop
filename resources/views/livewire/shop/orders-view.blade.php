<div class="min-h-screen w-full md:p-10" x-data="{ showSuccess: false }">
    <div class="w-full rounded-lg shadow-lg bg-cm-pink1 py-10 text-center" style="min-height: 44em">
        ...
        @if (count($orders) == 0)
            <div class="text-lg">
                <p>{{ Auth::user()->role_id == '1' ? 'No has hecho ningún pedido aún' : 'No hay ningún pedido aún' }}
                </p>
                @if (Auth::user()->role_id == '1')
                    <P>Visita nuestra tienda y selecciona los productos de tu agrado para realizar tu primer pedido:
                    </P>
                    <a href="{{ route('product.index') }}" class="hover:text-cm-main-pink"><i
                            class="fas fa-store text-4xl mt-4"></i><br>!Click
                        Aquí!</a>
                @endif
            </div>
        @else
